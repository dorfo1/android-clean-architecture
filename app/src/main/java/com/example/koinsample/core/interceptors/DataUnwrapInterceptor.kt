...
class DataUnwrapInterceptor : Interceptor {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        responseBody?.let {
            val json = try { JSONObject(it) } catch(e : Exception) { return response }
    ...
    private fun <T>buildNewResponseBody(json: T, contentType: MediaType?): ResponseBody {
        return ResponseBody.create(contentType,json.toString())
    }
